...
user="`whoami`"
stars=$(printf '%*s' 100 '')
txt="$user began Slurm job on $CLUSTER: `date`"
ch="#"
echo -e "${txt//?/$ch}\n${txt}\n${txt//?/$ch}"
echo "${stars// /*}"
...
# Usage: sbatch --export=LABEL_TYPE=single csc_finetuner.sh
...
# =============================================================================
# CLUSTER-SPECIFIC BATCH SIZE ADJUSTMENTS
# =============================================================================
# Determine batch size based on cluster, strategy, and architecture
if [ "$CLUSTER" = "mahti" ]; then
	# Mahti: A100 GPUs with more memory - can use larger batches
	case $strategy in
		"full"|"lora")
			case $architecture in
				"ViT-L/14@336px")
					ADJUSTED_BATCH_SIZE=64  # A100 can handle larger batches
					;;
				"ViT-L/14")
		...
		"progressive")
			case $architecture in
				"ViT-L/14@336px")
					...
					;;
				"ViT-L/14")
					ADJUSTED_BATCH_SIZE=192
					;;
				"ViT-B/32"|"ViT-B/16")
					ADJUSTED_BATCH_SIZE=384
					;;
			esac
			;;
		"probe")
			case $architecture in
				"ViT-L/14@336px")
					...
					;;
				"ViT-L/14")
					ADJUSTED_BATCH_SIZE=768
					;;
				"ViT-B/32"|"ViT-B/16")
					ADJUSTED_BATCH_SIZE=1536
					;;
			esac
			;;
	esac
elif [ "$CLUSTER" = "puhti" ]; then
	# Puhti: V100 GPUs with less memory - use conservative batches
	case $strategy in
		"full"|"lora")
...
# JOB COMPLETION
# =============================================================================
done_txt="$user finished Slurm job on $CLUSTER: $(date)"
echo -e "${done_txt//?/$ch}\n${done_txt}\n${done_txt//?/$ch}"