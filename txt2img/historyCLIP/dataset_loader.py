...
class HistoricalDataset(Dataset):
		def __init__(self, data_frame, captions, img_sz=28, txt_category="query", dataset_directory="path/2/images"):
				...
				# Preprocess images at initialization (optional)
				self.preprocess_images()

		def preprocess_images(self):
				"""Preprocess images by resizing and saving them to reduce size."""
				for idx in range(len(self.data_frame)):
						...
						if os.path.exists(img_path):
								try:
										image = Image.open(img_path).convert('RGB')
										# Resize and compress image before saving it back
										image = self.resize_and_pad(image, self.img_sz)
										# Save the preprocessed image back to disk (optional)
										image.save(img_path, format='JPEG', quality=85)  # Adjust quality as needed
