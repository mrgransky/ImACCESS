...
def simplified_progressive_finetune(
		...
		for epoch in range(num_epochs):
				...
				if verbose:
						print(f"Epoch {epoch+1:3d} | Phase {current_phase} | "
...
def create_phase_optimizer(model, lr, wd):
		"""
		Create a fresh optimizer for the current phase.
		"""
...
def print_phase_status(model, layers_to_unfreeze):
		"""
		Print current phase parameter status.
		"""
		total_params = sum(p.numel() for p in model.parameters())
		trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
		
		print(f"Phase status: {trainable_params:,}/{total_params:,} "
