...
plt.title(f'Label Distribution {train_df.shape[0]} Training samples {val_df.shape[0]} Validation Samples | Total samples: {merged_df.shape[0]}')
plt.xlabel('Label')
plt.ylabel('Frequency')
plt.legend(loc='best', ncol=2, frameon=False, fontsize=10)
plt.tight_layout()
plt.savefig('label_distribution_train_val.png')