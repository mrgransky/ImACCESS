...
# # Define semantic categories for better organization
# SEMANTIC_CATEGORIES = {
...
# # Add an expanded metadata stopwords list
# METADATA_PATTERNS = [
...
# 		print(f"Extracted {len(flat_topics)} unique topic terms")
# 		return topics, flat_topics

# def is_english(text):
# 	"""Improved English detection with character set analysis"""
...
# 	if total_chars > 0 and ascii_chars / total_chars < 0.9:
# 		return False
...
# 		words = text.lower().split()
# 		common_words = {'the', 'and', 'of', 'to', 'in', 'is', 'was'}
...
# 		# Common metadata fragments to remove
...
# 		# Get scores for the first document (there's only one)
# 		scores = zip(feature_names, X.toarray()[0])
# 		# Sort by score in descending order
# 		sorted_scores = sorted(scores, key=lambda x: x[1], reverse=True)
# 		# Return top keywords as a set
# 		top_k = 20  # Get more keywords initially for further filtering
# 		keywords = [kw for kw, score in sorted_scores[:top_k] if score > 0.01]
# 		return keywords
# 	except:
# 		return []
...
# 	# Step 1: Topic Modeling - discover global topics
...
# 		relevant_labels = handle_multilingual_labels(relevant_labels)
...
# 				relevant_labels.append(original_label.lower().strip())

# 		# Remove redundancy
# 		relevant_labels = deduplicate_labels(relevant_labels)
