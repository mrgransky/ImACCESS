...
def generate_labels(img_source: str = "/path/2/test_img/baseball.jpeg", rnd: int=11):
	print(f"Received {img_source} for image labling backend")

	# Output imge labels file
	output_labels_fpth = os.path.join(IMG_2_TXT_DIRECTORY, "outputs", f"labeled_img_x{rnd}.pkl")
	...
	command = f"cd {IMG_2_TXT_DIRECTORY} && bash run_img_labeling.sh {img_source} {output_labels_fpth}"
