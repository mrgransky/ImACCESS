...
REGIME_COLORS = {
		"AGREEMENT":        "#4CAF50",
		"SOFT_CONFLICT":    "#FF9800",
		"HARD_CONFLICT":    "#F44336",
		"MISSING_MODALITY": "#9E9E9E",
		"UNKNOWN":          "#607D8B",
}
...
def viz_regime_distribution(
		...
		for at in autotexts:
				at.set_fontsize(9)
		axes[0].legend(
				wedges, [f"{r}  ({regime_counts[r]:,})" for r in regimes],
...
def viz_similarity_histograms(
		audit_jsonl: str,
		sim_key: str = "cosine_sim",       # key inside each receipt
		out_dir: Optional[str] = None,
):
		"""
		...
		if not regime_sims:
				print(f"[VIZ][WARN] No '{sim_key}' field found in receipts. "
...
def viz_cgd_heatmap(
		...
		fig.suptitle("Stage 3 — Mean CGD Scores by Regime",
								 fontsize=14, fontweight="bold")
...
def viz_label_count_distribution(
		...
		for ax, regime in zip(axes, regimes):
				...
				ax.hist(sub["n_pos"], bins=bins, color=color, alpha=0.75,
								label=f"pos_targets (μ={sub['n_pos'].mean():.1f})", edgecolor="white")
				ax.hist(sub["n_hn"], bins=bins, color="#E53935", alpha=0.5,
								label=f"hn_targets  (μ={sub['n_hn'].mean():.1f})", edgecolor="white")
...
# ─────────────────────────────────────────────────────────────────────────────
# RUNNER — call all visualizations in one shot
# ─────────────────────────────────────────────────────────────────────────────

def run_all_visualizations(
		audit_jsonl:      str,           # Stage 2 output
		freq_json:        str,           # Bridge Step 2 global_label_frequency.json
		stage3_jsonl:     str,           # Stage 3 Micro-CGD output
		stage4_parquet:   str,           # Stage 4 auditable_supervision_matrix.parquet
		# Bridge funnel numbers — read from your Bridge printout:
		n_raw_concepts:     int = 172,
		n_after_clustering: int = 0,     # fill from Bridge Step 5 printout
		n_after_audit:      int = 0,     # fill from Bridge Step 8 printout
		n_target_vocab:     int = 0,     # fill from Bridge Step 10 printout
		n_emb_cache:        int = 0,     # fill from Bridge Step 11 printout
		out_dir: Optional[str] = None,   # set to a folder path to save PNGs
):
		print("[VIZ] V1 — Regime Distribution")
		viz_regime_distribution(audit_jsonl, out_dir=out_dir)

		print("[VIZ] V2 — Similarity Histograms")
		viz_similarity_histograms(audit_jsonl, out_dir=out_dir)

		print("[VIZ] V3 — Zipf Curve")
		viz_zipf_curve(freq_json, out_dir=out_dir)

		print("[VIZ] V4 — Vocabulary Funnel")
		viz_vocabulary_funnel(
				n_raw_concepts, n_after_clustering, n_after_audit,
				n_target_vocab, n_emb_cache, out_dir=out_dir,
		)

		print("[VIZ] V5 — CGD Heatmap")
		viz_cgd_heatmap(stage3_jsonl, out_dir=out_dir)

		print("[VIZ] V6 — Supervision Weights")
		viz_supervision_weights(stage4_parquet, out_dir=out_dir)

		print("[VIZ] V7 — Regime × Label Heatmap")
		viz_regime_label_heatmap(stage4_parquet, out_dir=out_dir)

		print("[VIZ] V8 — Label Count Distribution")
		viz_label_count_distribution(stage4_parquet, out_dir=out_dir)
...
def main():
		...
		parser.add_argument(
				...
				type=str,
				required=True,
				help="Path to Stage 2 Evidence Receipt JSONL "
						 "(e.g. test_mlm_cot_modality_conflict_audit.jsonl)",
		)
		parser.add_argument(
				"--freq_json", "-f",
				type=str,
				required=True,
				help="Path to Bridge Step 2 global_label_frequency.json",
		)
		parser.add_argument(
				"--out_dir", "-o",
				type=str,
				default=None,
				help="Directory to save PNG figures. If omitted, figures are only displayed.",
		)

		# ── Stage 3 / 4 paths (optional until those stages are complete) ──────────
		parser.add_argument(
				"--stage3_jsonl",
				type=str,
				default=None,
				help="Path to Stage 3 Micro-CGD Audit output JSONL. "
						 "Required for V5 (CGD heatmap). Skip to omit V5.",
		)
		parser.add_argument(
				"--stage4_parquet",
				type=str,
				default=None,
				help="Path to Stage 4 auditable_supervision_matrix.parquet. "
						 "Required for V6, V7, V8. Skip to omit those plots.",
		)
		...
		parser.add_argument(
				...
				default=None,
				metavar="V1,V2,...",
				help="Comma-separated list of visualizations to run "
						 "(e.g. --select V1,V3,V4). Runs all by default.",
		)
		parser.add_argument(
				"--sim_key",
				type=str,
		...
		if args.stage3_jsonl is None and "V5" in run_set:
				print("[VIZ][SKIP] --stage3_jsonl not provided → skipping V5 (CGD heatmap).")
				run_set.discard("V5")
		if args.stage4_parquet is None:
				for v in ("V6", "V7", "V8"):
						if v in run_set:
								print(f"[VIZ][SKIP] --stage4_parquet not provided → skipping {v}.")
								run_set.discard(v)
		...
		if "V1" in run_set:
				print("[VIZ] V1 — Regime Distribution")
				viz_regime_distribution(args.audit_jsonl, out_dir=args.out_dir)

		if "V2" in run_set:
				print("[VIZ] V2 — Similarity Histograms")
				viz_similarity_histograms(args.audit_jsonl, sim_key=args.sim_key, out_dir=args.out_dir)

		if "V3" in run_set:
				print("[VIZ] V3 — Zipf Curve")
				viz_zipf_curve(args.freq_json, top_n_labels=args.top_n_zipf, out_dir=args.out_dir)
		...
		if "V5" in run_set:
				print("[VIZ] V5 — CGD Heatmap")
				viz_cgd_heatmap(args.stage3_jsonl, out_dir=args.out_dir)

		if "V6" in run_set:
				print("[VIZ] V6 — Supervision Weights")
				viz_supervision_weights(args.stage4_parquet, out_dir=args.out_dir)

		if "V7" in run_set:
				print("[VIZ] V7 — Regime × Label Heatmap")
				viz_regime_label_heatmap(args.stage4_parquet, top_k=args.top_k_labels, out_dir=args.out_dir)

		if "V8" in run_set:
				print("[VIZ] V8 — Label Count Distribution")
				viz_label_count_distribution(args.stage4_parquet, out_dir=args.out_dir)
