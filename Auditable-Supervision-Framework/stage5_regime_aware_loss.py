...
def resolve_regime_weights(
		...
		for i, regime in enumerate(regimes):
				if regime in SKIP_REGIMES:
						...
						omega_neg[i]  = 0.0
						if verbose:
								print(f"[WARN][resolve_regime_weights] Unknown regime '{regime}' at idx {i}. "
...
def compute_stage5_loss(
		...
		"""
		Master Stage 5 loss combining three arms:
		...
		-------
		total_loss  : scalar
		loss_i2t    : scalar  (for logging)
		loss_t2i    : scalar  (for logging)
		loss_repel  : scalar  (for logging)
		"""
		...
		class_embeds = torch.nn.functional.normalize(
				all_class_embeds, dim=-1
		)                                                   # [C, D] already FP32
		...
		# ── Combine ───────────────────────────────────────────────────────────────
		total_loss = (
					loss_weights["i2t"]   * loss_i2t
				+ loss_weights["t2i"]   * loss_t2i
				+ loss_weights["repel"] * loss_repel
		)
