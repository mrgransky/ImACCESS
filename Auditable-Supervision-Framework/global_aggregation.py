...
def cluster_and_save_priors_(
	...
	malformed_count  = 0
	empty_pool_count = 0
	gmm_features: List[List[float]] = [] # Collect continuous 3D features for unsupervised GMM training
		...
		for line_no, line in enumerate(f, start=1):
			...
			if regime in ("AGREEMENT", "HARD_CONFLICT", "SOFT_CONFLICT"):
				...
				if metrics:
					...
					# Only append if all metrics are valid floats (safeguard)
					if set_sim is not None and orphan_ratio is not None and asym_gap is not None:
						gmm_features.append([set_sim, orphan_ratio, abs(asym_gap)])
	...
	# UNSUPERVISED CONFLICT REGIME INDUCTION (GMM)
	print(f"\n{'─'*80}")
	print(f"[BRIDGE][STEP 1] Unsupervised Conflict Regime Induction (GMM)")
	print(f"{'─'*80}")

	gmm_path = os.path.join(outputs_dir, f"{stem}_conflict_gmm.pkl")

	if len(gmm_features) >= 3:
		features_arr = np.array(gmm_features)
		scaler = StandardScaler()
		features_scaled = scaler.fit_transform(features_arr)
		print(f"[BRIDGE][STEP 1] Fitting 3-Component GMM over {len(features_scaled)} scaled continuous 3D metric vectors...")
		
		# Fit the unsupervised GMM
		gmm = GaussianMixture(n_components=3, covariance_type='full', random_state=42)
		gmm.fit(features_scaled)
		
		# Symmetrical centroid mapping (Resolves random cluster ID assignment)
		means = gmm.means_ # Shape: [3 clusters, 3 features] (set_sim, orphan_ratio, abs_gap)
		
		# 1. HARD_CONFLICT cluster has the highest orphan_ratio (column index 1)
		hard_cluster_idx = int(np.argmax(means[:, 1]))
		
		# 2. Of the remaining two, the one with the highest asymmetry_gap (column index 2) is SOFT_CONFLICT
		remaining_indices = [i for i in range(3) if i != hard_cluster_idx]
		soft_cluster_idx = remaining_indices[0] if means[remaining_indices[0], 2] > means[remaining_indices[1], 2] else remaining_indices[1]
		
		# 3. The last remaining cluster is AGREEMENT
		agree_cluster_idx = [i for i in range(3) if i not in [hard_cluster_idx, soft_cluster_idx]][0]
		...
		# Package the model and its deterministic mappings
		gmm_payload = {
			"gmm_model": gmm,
			"scaler": scaler,
		...
		# Serialize GMM payload to outputs/ folder
		joblib.dump(gmm_payload, gmm_path)
		print(f"[BRIDGE][STEP 1] Saved unsupervised conflict GMM model → {gmm_path}")
		
		# Print GMM Centroids for Verification / LaTeX reporting
		print(f"[BRIDGE][STEP 1] Discovered Cluster Centroids [set_sim, orphan_ratio, abs_gap]:")
		print(f"  ├─ AGREEMENT (Cluster {agree_cluster_idx:d})    : "
				f"[{means[agree_cluster_idx][0]:.3f}, {means[agree_cluster_idx][1]:.3f}, {means[agree_cluster_idx][2]:.4f}]")
		print(f"  ├─ SOFT_CONFLICT (Cluster {soft_cluster_idx:d}) : "
				f"[{means[soft_cluster_idx][0]:.3f}, {means[soft_cluster_idx][1]:.3f}, {means[soft_cluster_idx][2]:.4f}]")
		print(f"  └─ HARD_CONFLICT (Cluster {hard_cluster_idx:d}) : "
				f"[{means[hard_cluster_idx][0]:.3f}, {means[hard_cluster_idx][1]:.3f}, {means[hard_cluster_idx][2]:.4f}]")
	else:
		print(f"[BRIDGE][STEP 1] [WARN] Insufficient samples ({len(gmm_features)}) to train GMM. Skipping GMM generation.")
