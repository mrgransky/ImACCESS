...
class RegimeAwareDataset(Dataset):
	...
	def __repr__(self) -> str:
		...
		return (
			f"\n{self.dataset_name}\n"
			f"\t{self.split.upper()} :  ({len(self.data_frame):,} samples)\n"
...
def customized_collate_fn(batch: List[Dict]) -> Dict[str, Any]:
	"""
...
def get_stage5_dataloaders(
	...
	if cache_size is None:
		...
		avg_img_mb    = get_estimated_image_size_mb(
			image_paths=(df_train["img_path"].tolist() + df_val["img_path"].tolist()),
	...
	# Train & Val Datasets:
	train_dataset = RegimeAwareDataset(
	...
	val_dataset = RegimeAwareDataset(
		dataset_name=dataset_name,
		train=False,
