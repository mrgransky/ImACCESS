...
def evaluate_validation_set(
		...
		Uses cached embeddings for initial efficiency but recomputes embeddings and logits to ensure
		both in-batch and full-set metrics reflect the current model state.
		...
		if cache_file and os.path.exists(cache_file):
				...
				# Recompute embeddings and loss with current model
				image_embeds_list = []
				text_embeds_list = []
				with torch.no_grad():
						for bidx, (images, tokenized_labels, labels_indices) in enumerate(validation_loader):
								...
								for start in range(0, batch_size, chunk_size):
										...
										# Cosine similarity with current embeddings
						...
						# Concatenate recomputed embeddings
		...
		# Recompute class text embeddings for full-set metrics
		with torch.no_grad():
				text_inputs = clip.tokenize(class_names).to(device, non_blocking=True)
				with torch.amp.autocast(device_type=device.type, enabled=True):
						class_text_embeds = model.encode_text(text_inputs)
		...
		# Compute metrics with updated embeddings
		in_batch_metrics = compute_in_batch_metrics(
				validation_loader, all_image_embeds, all_text_embeds, all_labels, in_batch_loss, num_batches,
