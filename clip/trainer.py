...
def linear_probe_finetune_multi_label(
		...
		"""
		Linear probing fine-tuning for multi-label CLIP classification.
		Freezes encoders and trains a linear probe on image embeddings.
		
		This hybrid approach combines simplicity with efficiency options.
		"""
		# Set default loss weights
		if loss_weights is None:
				loss_weights = {"i2t": 0.5, "t2i": 0.5}
		...
		# Create linear probe
		if probe_hidden_dim is not None:
				# MLP probe
				linear_probe = torch.nn.Sequential(
						torch.nn.Linear(embed_dim, probe_hidden_dim),
						torch.nn.ReLU(),
						torch.nn.Dropout(probe_dropout),
						torch.nn.Linear(probe_hidden_dim, num_classes)
				).to(device)
				probe_type = "MLP"
		else:
				# Simple linear probe
				linear_probe = torch.nn.Linear(embed_dim, num_classes).to(device)
				probe_type = "Linear"
		...
		# Initialize probe with CLIP text embeddings (helps convergence for linear probe, optional for MLP probe)
		if probe_type == "Linear":
				with torch.no_grad():
						linear_probe.weight.data = all_class_embeds.clone() * 0.1
						linear_probe.bias.data.zero_()
				print("Initialized probe with scaled CLIP text embeddings")
		else:
				print("Initialized probe with random weights")
				pass
...
def linear_probe_finetune_multi_label_old(
		model: torch.nn.Module,
		train_loader: DataLoader,
		validation_loader: DataLoader,
		num_epochs: int,
		...
		Args same as lora_finetune_multi_label(), excluding LoRA params.
		"""
		# Set default loss weights
		if loss_weights is None:
				loss_weights = {"i2t": 0.5, "t2i": 0.5}
		...
		# Add linear probe on image embeddings
		embed_dim = model.visual.output_dim  # Assuming CLIP's image encoder output dim
		linear_probe = torch.nn.Linear(embed_dim, num_classes).to(device)
		...
		training_losses = []
		training_losses_breakdown = {"total": [], "i2t": [], "t2i": []}
		img2txt_metrics_all_epochs = []
		txt2img_metrics_all_epochs = []
		in_batch_loss_acc_metrics_all_epochs = []
		full_val_loss_acc_metrics_all_epochs = []
		...
		for epoch in range(num_epochs):
				...
				print(
						f'@ Epoch {epoch + 1}:\n'
						f'\t[LOSS] {mode}:\n'
						f'\t\tTraining - Total: {avg_total_loss:.6f}\n'
						f'\t\tValidation: {current_val_loss:.6f}\n'
						f'\tMulti-label Validation Metrics:\n'
						f'\t\tIn-batch Top-K Accuracy:\n'
						f'\t\t\t[Image→Text]: {in_batch_loss_acc_metrics_per_epoch.get("img2txt_topk_acc")}\n'
				...
				if early_stopping.should_stop(
						current_value=current_val_loss,
						model=linear_probe,  # Save/restore linear probe instead of full model
						epoch=epoch,
						optimizer=optimizer,
						scheduler=scheduler,
						checkpoint_path=mdl_fpth,
