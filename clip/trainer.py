...
def evaluate_validation_set(
    ...
    if cache_exists:
        ...
        if not (all_labels.min() >= 0 and all_labels.max() < n_classes):
            raise ValueError(
                f"Cached labels invalid. Expected range [0, {n_classes-1}], "
                f"found min={all_labels.min()}, max={all_labels.max()}, "
                f"unique={sorted(set(all_labels.tolist()))}"
            )
        ...
        with torch.no_grad():
            for bidx, (images, tokenized_labels, labels_indices) in enumerate(validation_loader):
                ...
                if not (labels_indices.min() >= 0 and labels_indices.max() < n_classes):
                    raise ValueError(
                        f"Batch {bidx+1}: labels_indices out of range [0, {n_classes-1}], "
                        f"min={labels_indices.min()}, max={labels_indices.max()}, "
                        f"unique={sorted(set(labels_indices.tolist()))}"
                    )
                for start in range(0, batch_size, chunk_size):
                    ...
                    if not (chunk_labels.min() >= 0 and chunk_labels.max() < n_classes):
                        raise ValueError(
                            f"Batch {bidx+1}, chunk {start}-{end}: chunk_labels out of range [0, {n_classes-1}], "
                            f"min={chunk_labels.min()}, max={chunk_labels.max()}, "
                            f"unique={sorted(set(chunk_labels.tolist()))}"
                        )

                    with torch.amp.autocast(device_type=device.type, enabled=True):
                        logits_per_image, logits_per_text = model(chunk_images, chunk_texts)

                    ground_truth = chunk_labels
                    ...
                    img2txt_pred = logits_per_image.argmax(dim=1)
                    txt2img_pred = logits_per_text.argmax(dim=1)
                    img2txt_correct += (img2txt_pred == ground_truth).sum().item()
                    txt2img_correct += (txt2img_pred == ground_truth).sum().item()

                    for k in topK_values:
                        if k <= chunk_size_actual:
                            topk_img2txt = logits_per_image.topk(k, dim=1)[1]
                            topk_txt2img = logits_per_text.topk(k, dim=1)[1]
                            img2txt_topk_correct[k] += sum(ground_truth[i].item() in topk_img2txt[i] for i in range(chunk_size_actual))
                            txt2img_topk_correct[k] += sum(ground_truth[i].item() in topk_txt2img[i] for i in range(chunk_size_actual))
        ...
        with torch.no_grad():
            ...
            for bidx, (images, tokenized_labels, labels_indices) in enumerate(validation_loader):
                batch_size = images.size(0)
                # Validate labels_indices immediately
                if not (labels_indices.min() >= 0 and labels_indices.max() < n_classes):
                    raise ValueError(
                        f"Batch {bidx+1}: labels_indices out of range [0, {n_classes-1}], "
                        f"min={labels_indices.min()}, max={labels_indices.max()}, "
            ...
            if not (all_labels.min() >= 0 and all_labels.max() < n_classes):
                raise ValueError(
                    f"Collected labels out of range [0, {n_classes-1}], "
                    f"min={all_labels.min()}, max={all_labels.max()}, "
                    f"unique={sorted(set(all_labels.tolist()))}"
                )
    ...
    # Recompute class text embeddings
    with torch.no_grad():
        text_inputs = clip.tokenize(class_names).to(device, non_blocking=True)
        with torch.amp.autocast(device_type=device.type, enabled=True):
            class_text_embeds = model.encode_text(text_inputs)
    ...
    # Full-set metrics
    similarity_i2t = all_image_embeds.to(device) @ class_text_embeds.to(device).T
    img2txt_pred = similarity_i2t.argmax(dim=1)
    img2txt_acc = (img2txt_pred.cpu() == all_labels).float().mean().item()

    img2txt_topk_acc = {}
    valid_k_values = [k for k in topK_values if k <= n_classes]
    for k in valid_k_values:
        topk_indices = similarity_i2t.topk(k, dim=1)[1]
        correct = (topk_indices == all_labels.unsqueeze(1).to(device)).any(dim=1)
        img2txt_topk_acc[k] = correct.float().mean().item()

    similarity_t2i = class_text_embeds.to(device) @ all_image_embeds.to(device).T
    txt2img_pred = similarity_t2i.argmax(dim=1)
    txt2img_acc = (txt2img_pred.cpu() == torch.arange(n_classes, device=device)).float().mean().item()

    txt2img_topk_acc = {}
    for k in topK_values:
        topk_indices = similarity_t2i.topk(min(k, total_samples), dim=1)[1].cpu()
        correct = sum(1 for i in range(n_classes) if i in all_labels[topk_indices[i]])
        txt2img_topk_acc[k] = correct / n_classes

    ranks = similarity_i2t.argsort(dim=1, descending=True)
    rr_indices = ranks.eq(all_labels.view(-1, 1).to(device)).nonzero(as_tuple=True)[1] + 1
    ...
    img2txt_metrics = get_retrieval_metrics(
        ...
        query_labels=all_labels.to(device),
    ...
    txt2img_metrics = get_retrieval_metrics(
        ...
        candidate_labels=all_labels.to(device),
        topK_values=topK_values,
        mode="Text-to-Image",
