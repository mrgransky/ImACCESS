...
def plot_qualitative_retrieval_i2t(
	...
	verbose: bool = False,
		...
		# Figure dimensions
		IMG_COL_W  = 1.0   # inches — query image column
		GT_COL_W   = 1.3   # inches — ground truth text column
		CELL_W     = 2.2   # inches — per strategy column
		ROW_H      = 1.6   # inches — per image row
		HEADER_H   = 0.55  # inches — strategy header row
		
		# Calculate total width with the extra column
		fig_w = IMG_COL_W + GT_COL_W + n_strat * CELL_W + 0.3
		fig_h = HEADER_H + num_images * ROW_H + 0.4
...
def plot_qualitative_retrieval_t2i(
	...
	output_dir : str
	topk : int
		Number of retrieved images shown per cell (usually 1).
	verbose : bool
	Returns
	-------
	str  Path to saved figure.
	...
	strategies = list(results_by_strategy.keys())
	if not strategies:
		print("[plot_qualitative_retrieval_t2i] No strategies provided — skipping.")
		return ""
	ref_t2i = results_by_strategy[strategies[0]]["t2i"]
	
	num_labels = len(ref_t2i)
	if num_labels == 0:
		print("[plot_qualitative_retrieval_t2i] No T2I samples — skipping.")
		return ""
	...
	for i, sample_ref in enumerate(ref_t2i):
			...
			for j, strat in enumerate(strategies):
					...
					match = next(
							(s for s in strat_data if s["query_label"] == query_label),
							None,
					)
					if match is None or not match["retrieved_paths"]:
							ax.set_facecolor("#EEEEEE")
							ax.text(0.5, 0.5, "–", ha="center", va="center",
											fontsize=_ANNO_SIZE, transform=ax.transAxes)
							continue
					...
					if img_arr is not None:
							...
							ax.text(0.5, 0.6, "N/A", ha="center", va="center",
											fontsize=_ANNO_SIZE, transform=ax.transAxes)
	...
	fig.suptitle(
		f"T2I Qualitative Top-{topk} Retrieval — Tail Distribution",
		fontsize=_TITLE_SIZE + 0.5,
		fontweight="bold",
		y=0.995,
	)
...
def plot_unfreeze_heatmap(
		...
		if last_phase < (n_phases - 1):
				...
				legend_handles.append(Patch(color="#A3A3A3", alpha=0.6, label="Unused Phases"))
...
def plot_image_to_texts_pretrained(
	...
	gs = GridSpec(1, 2, width_ratios=[1, 1.05], wspace=0.01)
...
def visualize_samples(dataloader, num_samples=5):
	...
	# Build inverse mapping: class index -> label name
	idx_to_label = {v: k for k, v in dataset.label_dict.items()}
	for bidx, (images, tokenized_labels, labels_indices) in enumerate(dataloader):
			...
			for i in range(min(dataloader.batch_size, batch_size)):
					global_idx = start_idx + i
					if global_idx >= len(dataset):
							break
					image = images[i].permute(1, 2, 0).numpy()
					label_vector = labels_indices[i]  # shape [num_classes]
					# Get active label indices (multi-hot)
					active_indices = (label_vector > 0).nonzero(as_tuple=True)[0].cpu().tolist()
					active_labels = [idx_to_label[j] for j in active_indices]
					path = dataset.images[global_idx]
					raw_label_str = dataset.labels[global_idx]  # original string like "['foo', 'bar']"
					print(f"Global Index: {global_idx}")
					print(f"Image {image.shape} Path: {path}")
					print(f"Raw label string: {raw_label_str}")
					print(f"Active label indices: {active_indices}")
					print(f"Active label names: {active_labels}")

					# Denormalize using this dataset's normalization
					mean = np.array([0.480122467021897, 0.47967717137128263, 0.4836431951614201])
					std = np.array([0.2789868562674993, 0.27836950211489425, 0.27876165990706897])
					...
					title_labels = ", ".join(active_labels)  # truncate if many labels
					if len(active_labels) > 7:
						title_labels += " ..."
					plt.title(f"{title_labels}", fontsize=8)
					plt.axis('off')
					plt.tight_layout()
					plt.savefig(f"sample_{global_idx}.png", dpi=300, bbox_inches='tight')
					plt.close()
...
def plot_retrieval_metrics_best_model(
	...
	for i, metric in enumerate(metrics):
		...
		it_top_ks = list(map(int, image_to_text_metrics[metric].keys()))
...
def plot_retrieval_metrics_per_epoch(
	...
	if image_to_text_metrics_list and text_to_image_metrics_list:
		...
		it_valid_k_values = sorted([int(k) for k in it_first_metrics.keys()])
...
def plot_qualitative_retrieval(
	...
	# Legend
	handles = [
		Patch(color="#2ca02c", label="Correct (in ground truth)"),
