...
def _qwen_vlm_(response: str, verbose: bool=False) -> Optional[List[str]]:		
	...
	def split_items_relaxed(list_like: str) -> List[str]:
		...
		if verbose: print(f"[DEBUG] split_items_relaxed input: {repr(list_like)}")
		...
		if verbose: print(f"[DEBUG] Split by comma into {len(parts)} parts: {parts}")
		...
		if verbose: print(f"[DEBUG] split_items_relaxed output: {items}")
		return [x for x in items if x]
	
	def dedupe_preserve_order(items: List[str], limit: int = 5) -> List[str]:
		"""Remove duplicates while preserving order, cap at limit."""
		if verbose: print(f"[DEBUG] dedupe_preserve_order input ({len(items)} items): {items}")
